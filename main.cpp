...
void getPassword(const Passwords* p_Database, const int numEntries, const string& application) {
    ...
    if (!found) {
        cout << "Password entry not found for application: " << application << endl;
    }
}
...
void deletePassword(Passwords* new_pass, int& entriesCount, const string& application) {
    for (int i = 0; i < entriesCount; ++i) {
        if (new_pass[i].application == application) {
            ...
            entriesCount--;
            cout << "Password entry deleted successfully for application: " << application << endl;
            return;
        }
    }
    cout << "Password entry not found for application: " << application << endl;
...
void changePassword(Passwords* new_pass, const int entriesCount, const string& application, const string& key) {
    for (int i = 0; i < entriesCount; ++i) {
        if (new_pass[i].application == application) {
            ...
            cout << "Password changed successfully for application: " << application << endl;
...
int main() {
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 4: {
                string application;
                cout << "Enter application to change password: ";
                cin >> application;
